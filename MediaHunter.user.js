...
(function () {
    ...
    // 纯净内容透视方案：用于标记内容实为 M3U8/MPD 但被伪装的原始 URL
    const FORCE_M3U8_URLS = new Set();
    const FORCE_MPD_URLS = new Set();
        ...
        for (let i = 0; i < data.length - 4; i++) {
            if (data[i] === 0x00 && data[i + 1] === 0x00 && data[i + 2] === 0x01 && (data[i + 3] & 0x1F) === 7) {
                ...
                try {
                    ...
                    else if (pic_order_cnt_type === 1) continue;
                    ...
                    if (frame_cropping_flag) {
                        ...
                        if (crop_l > 50 || crop_r > 50 || crop_t > 50 || crop_b > 50) continue;
                    }
                    ...
                    // 校验分辨率的合理性。正常视频宽度与高度应在 32 到 8192 之间，且宽高比在合理范围内（不大于 10 且不小于 0.1）
                    if (width >= 32 && width <= 8192 && height >= 32 && height <= 8192 && (width / height < 10) && (height / width < 10)) {
                        return { width, height };
                    }
                } catch (e) {
                    // 解析异常则跳过，继续寻找下一个可能的 SPS
                }
            }
        }
        return null;
    ...
    // 流式读取 Response.body 的前 1KB 字节以检测内容特征
    async function sniffStreamSignature(responseClone) {
        if (!responseClone.body || typeof responseClone.body.getReader !== 'function') return null;
        const reader = responseClone.body.getReader();
        const chunks = [];
        let receivedLength = 0;
        try {
            while (receivedLength < 1024) {
            ...
            if (receivedLength === 0) return null;
    ...
    function detectFormat(url, contentType) {
        // [核心修复点] 强制内容透视映射：如果在标记池中，无视一切强制赋予对应流媒体属性
        if (FORCE_M3U8_URLS.has(url)) {
            return { type: 'application/vnd.apple.mpegurl', ext: 'm3u8' };
        }
        if (FORCE_MPD_URLS.has(url)) {
            return { type: 'application/dash+xml', ext: 'mpd' };
        }
    ...
    // 解密并提取 Next.js 数据节点 (__NEXT_DATA__) 中伪装的媒体链接
    function parseNextData() {
        ...
        try {
            ...
            if (ev && ev.d && ev.k) {
                // 动态字符偏移还原算法
                const decryptedStr = atob(ev.d).split("").map(c => String.fromCharCode(c.charCodeAt(0) - ev.k)).join("");
                const decrypted = JSON.parse(decryptedStr);
                if (decrypted && decrypted.videoUrl) {
                    // 将伪装成图片的视频流地址加入强 M3U8 白名单以穿透过滤忽略机制
                    FORCE_M3U8_URLS.add(decrypted.videoUrl);

                    let resStr = '720P';
                    if (video?.sources?.[0]?.resolution) {
                        resStr = `${video.sources[0].resolution}P`;
                    }

                    handleMediaFound({
                        url: decrypted.videoUrl,
                        type: 'video',
                        ext: 'm3u8',
            ...
            console.warn("MediaHunter: Failed to parse and decrypt __NEXT_DATA__", e);
        }
    }
    ...
    function setupNetworkSniffing() {
        ...
        window.fetch = async function (input, init) {
            ...
            try {
                ...
                // 大文件快速登记通道：若明确大于 3MB，说明必定非M3U8/MPD索引文本，直接放行登记以避免多余特征匹配
                ...
                if (isManifest || isTextOrJson) {
                    // 若原本就是流媒体格式或文本/JSON，完整读取文本并处理（用于提取网页或JSON接口中隐藏的媒体链接）
                    clone.text().then(text => {
                        if (text && typeof text === 'string') {
                            const trimmed = text.trim();
                            if (trimmed.startsWith('#EXTM3U')) {
                                FORCE_M3U8_URLS.add(url);
                            } else if (trimmed.includes('<MPD') || trimmed.includes('<Period')) {
                                FORCE_MPD_URLS.add(url);
                            }
                        }

                        if (url.includes('.m3u8') || FORCE_M3U8_URLS.has(url) || url.includes('.mpd') || FORCE_MPD_URLS.has(url)) {
                    ...
                    // 对可疑被伪装的资源进行 1KB 流式响应特征检测，以节省带宽和内存消耗
                    sniffStreamSignature(clone).then(headerText => {
                        if (headerText) {
                            const isM3u8Sig = headerText.includes('#EXTM3U');
                            const isMpdSig = headerText.includes('<MPD') || headerText.includes('<Period');

                            if (isM3u8Sig || isMpdSig) {
                                if (isM3u8Sig) FORCE_M3U8_URLS.add(url);
                                if (isMpdSig) FORCE_MPD_URLS.add(url);

                                // 异步获取该资源的完整内容用于解析和文本提取
                                requestText(url).then(fullText => {
                                    if (fullText) {
                                        if (INTERCEPTED_TEXTS.size >= MAX_INTERCEPT_CACHE) {
                                            INTERCEPTED_TEXTS.delete(INTERCEPTED_TEXTS.keys().next().value);
                                        }
                                        INTERCEPTED_TEXTS.set(url, fullText);
                                        extractUrlsFromTextAsync(fullText, reqSession, true);
                                    }
                                    checkNetworkResource(url, contentType, reqSession);
                                }).catch(() => {
                                    checkNetworkResource(url, contentType, reqSession);
                        ...
                        // 匹配失败或读取异常，则作为常规网络资源直接检测（会被过滤机制忽略）
                        checkNetworkResource(url, contentType, reqSession);
                    }).catch(() => {
                        checkNetworkResource(url, contentType, reqSession);
                    });
                ...
                if (this.readyState === 4) {
                    ...
                    try {
                        ...
                        if (isManifest || isTextOrJson) {
                            if (this.responseType === '' || this.responseType === 'text') {
                                ...
                                try {
                                    ...
                                    if (text) {
                                        ...
                                        checkNetworkResource(url, contentType, this._reqSession);
                                        extractUrlsFromTextAsync(text, this._reqSession, true);
                                    } else {
                                        checkNetworkResource(url, contentType, this._reqSession);
                                    }
                                } catch (err) {
                            ...
                            // 对原本不是Manifest或Text/Json的可疑伪装资源（如伪装成图片的XHR请求）进行同步前 1KB 字节的特征匹配
                            if ((this.responseType === '' || this.responseType === 'text') && this.responseText) {
                                const headerText = this.responseText.substring(0, 1024);
                                const isM3u8Sig = headerText.includes('#EXTM3U');
                                const isMpdSig = headerText.includes('<MPD') || headerText.includes('<Period');
            ...
            try {
                const observer = new PerformanceObserver((list) => {
                    ...
                    list.getEntries().forEach(entry => {
                        // 允许处理脚本初始化前已缓冲的早期资源，直接交给 checkNetworkResource 过滤与排重
    ...
    async function checkNetworkResource(url, contentType, sourceSessionId = PAGE_SESSION_ID) {
        ...
        if (REGEX_IGNORE_URLS.test(lowerUrl) && !FORCE_M3U8_URLS.has(url) && !FORCE_MPD_URLS.has(url)) return;
    ...
    function getRenderableMedia() {
        ...
        if (sniffedList.length > 1) {
            displayList = displayList.filter(m => {
                ...
                // 如果是主流媒体（m3u8, mpd, flv），即使没有解析出分辨率，也绝不能在 UI 渲染时过滤掉！
                const isMainStream = ['m3u8', 'mpd', 'flv'].includes(m.ext);
                if (!m.resolution && !isMainStream) return false;
    ...
    // 运行 Next.js __NEXT_DATA__ 节点数据提取器
    const runNextDataExtractor = (sessionId) => {
    ...
    function setupSPAHandling() {
        ...
        const handleUrlChange = () => {
            if (window.location.href !== lastUrl) {
                ...
                runNextDataExtractor(PAGE_SESSION_ID);
                startDOMScanner();
                scheduleRender();
            }
        };
