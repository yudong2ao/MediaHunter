...
现代浏览器由于其严格的 DPAPI 级本地加密与独占锁，容易造成外部工具读取 Cookie 失败。本项目默认优先适配机制更为开放的 **Firefox**。

如果您倾向于使用 Chrome 或 Edge 作为获取 Cookie 与登录状态的主力浏览器，请接受下载能力有所降低的事实，并在 `yt-dlp.conf` 中调整相关的 Firefox 配置项。
